...
class Fuzzer:
    ...
    def run(self):
        ...
        with open(file="CodeDistance.json", mode="r", encoding="utf-8") as f1:
            code_distance = json.load(f1)

        with open(file="StateDistance.json", mode="r", encoding="utf-8") as f2:
            try:
                state_distance = json.load(f2)
            except json.decoder.JSONDecodeError:
                state_distance = {}

        # Create and run our evolutionary fuzzing engine
        engine = EvolutionaryFuzzingEngine(population=population, selection=selection, crossover=crossover, mutation=mutation, mapping=get_function_signature_mapping(self.env.abi), srcmap = self.srcmap)
        engine.fitness_register(lambda x: fitness_function(x, self.env))
        engine.analysis.append(ExecutionTraceAnalyzer(self.env, code_distance, state_distance))
