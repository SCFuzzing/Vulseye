...
class BlockDependencyDetector():
    ...
    def detect_block_dependency(self, tainted_record, current_instruction, previous_branch, transaction_index):
        ...
        #TODO
        if current_instruction and current_instruction["op"] in ["LT", "GT", "SLT", "SGT", "EQ"]:
                ...
                if tainted_record.stack[-2]:
                    for expression in tainted_record.stack[-2]:
                        if "blockhash" in str(expression) or \
                           ...
                           self.block_dependency = True
        #TODO
        elif current_instruction["op"] == "CALL" and (convert_stack_value_to_int(current_instruction["stack"][-3]) or tainted_record and tainted_record.stack[-3]) or \
        ...
        # Register block related information
            ...
            #print("set block_instruction in 4\n\n\n")
        # Check if execution stops withour exception
        #TODO
        if self.block_dependency and self.send_ether and current_instruction["op"] in ["STOP", "SELFDESTRUCT", "RETURN"]:
