...
def _get_targets_json(compilation_unit: "CompilationUnit", target: str, **kwargs: Any) -> Dict:
    ...
    #return compile_use_solcx(target,compilation_unit,solc,solc_arguments,solc_disable_warnings)


# def compile_use_solcx(target,compilation_unit,solc,solc_arguments,env):
#     compilation_unit.compiler_version = CompilerVersion(
#     compiler="solc", version='v0.8.0', optimized=is_optimized(solc_arguments))
#     solcx.set_solc_version('0.8.0')
#     output = solcx.compile_files([target])
#     print(output)
#     return output
...
def solc_handle_contracts(
    ...
    if "contracts" in targets_json:
        ...
        for original_contract_name, info in targets_json["contracts"].items():
            ...
            #print("info[\"abi\"]",str(info["abi"]))
            #print(is_above_0_8)

            # _info = json.dumps(info["abi"])

            # source_unit.abis[contract_name] = (
            #     json.loads(_info) if not is_above_0_8 else info["abi"]
            # )
            source_unit.abis[contract_name] = (
                info["abi"]
            )
...
def _is_at_or_above_minor_version(compilation_unit: "CompilationUnit", version: int) -> bool:
    ...
    solcx_version = int(str(solcx.get_solc_version()).split(".")[1])
    #return int(compilation_unit.compiler_version.version.split(".")[1]) >= version
    return solcx_version >= version
...
def _run_solc(
    ...
    # solcx.set_solc_version('0.8.0')
    # output = solcx.compile_files([filename])
        ...
        for line in lines:
            ...
            if 'pragma solidity' in line:
                ...
                if re.findall(pattern, line):
                    ...
                    solc_version = '0.8.0'
    print('=========================compiler_version:',compiler_version.version)
    print('=========================solidity version:',solc_version)
    installed_versions = []
    ...
    #print("output[ctc_fuc_key]", output[ctc_fuc_key])

    # for key in output[ctc_fuc_key]:
    #     print("key:", key)

    abi = output[ctc_fuc_key]['abi']
    #print('===================================================abi:\n',abi)

    bin = output[ctc_fuc_key]['bin']
    #print('===================================================bin:\n',bin)

    bin_runtime = output[ctc_fuc_key]['bin-runtime']
    #print('===================================================bin-runtime:\n',bin_runtime)

    devdoc = output[ctc_fuc_key]['devdoc']
    #print('===================================================devdoc:\n',devdoc)

    if 'hashes' in output[ctc_fuc_key].keys():
        hashes = output[ctc_fuc_key]['hashes']
        #print('===================================================hashes:\n',hashes)
    else:
        hashes = ''


    srcmap = output[ctc_fuc_key]['srcmap']
    #print('===================================================srcmap:\n',srcmap)

    srcmap_runtime = output[ctc_fuc_key]['srcmap-runtime']
    #print('===================================================srcmap-runtime:\n',srcmap_runtime)

    userdoc = output[ctc_fuc_key]['userdoc']
    #print('===================================================userdoc:\n',userdoc)

    filename = ctc_fuc_key.split(':')[0]
    #print('===================================================filename:\n',filename)

    aST = output[ctc_fuc_key]['ast']
    #print('===================================================AST:\n',aST)

    version_str = output[ctc_fuc_key]['metadata']
    pattern = r'\"(\d+\..+commit\..+?)\"'
    try:
        version = re.findall(pattern, version_str)[0]+'.Linux.g++'
    except:
    ...
    final_out: Dict = {
        'contracts':{
            ctc_fuc_key: {'abi':abi, 'bin':bin, 'bin-runtime':bin_runtime, 'devdoc':devdoc, 'hashes':hashes, 'srcmap':srcmap, 'srcmap-runtime':srcmap_runtime, 'userdoc':userdoc}
        },
        'sourceList':[filename],
        'sources':{filename:{'AST':aST}},
        'version':version
    }
    #print(final_out)
    return final_out
