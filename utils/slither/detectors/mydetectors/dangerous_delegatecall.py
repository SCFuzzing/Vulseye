...
from slither.analyses.data_dependency.data_dependency import is_tainted_remastered

class DangerousDelegateCallNew(AbstractDetector):

    ARGUMENT = "dangerous-delegatecall"
    HELP = ""
                    ...
                    for ir in node.irs:
                        if isinstance(ir, LowLevelCall):
                            if ir.function_name in ["delegatecall","callcode"]: 
                                if is_tainted_remastered(ir.arguments[0], function.contract):
                                    result.append(node)

                if result:
                    info += ["Dangerous delegatecall in ", function, ":\n"]

                    info += ["Delegatecall was tainted by dangerous input:\n"]
                    for node in result:
                        spot = ("Danger-Dele ", str(node.source_mapping.filename.absolute) + " ",
