...
from utils.utils import initialize_logger, convert_stack_value_to_int, convert_stack_value_to_hex, \
...
def var_distance(var, range_list):
    min_distance = math.inf
    for _range in range_list:
        x, y = _range
        if var < x:
            curr_distance = abs(x - var)
        elif var > y:
            curr_distance = abs(var - y)
        else:
            return 0
...
class ExecutionTraceAnalyzer(OnTheFlyAnalysis):
    def __init__(self, fuzzing_environment: FuzzingEnvironment, code_distance, state_distance) -> None:
        ...
        self.interesting_slot = set()
        self.interesting_value = set()
        self.initiate_distance = {}

        for target in self.state_distance:
            # target:236
            self.initiate_distance[target] = {}
            for path_index, path in enumerate(self.state_distance[target]):
                # path:{"1": [[4.0, 8.0]]}
                print(f"path_index:{path_index},path:{path}")
                self.initiate_distance[target][path_index] = [{}]
                for slot in path:
                    self.interesting_slot.add(int(slot))
                    self.initiate_distance[target][path_index][0][int(slot)] = 0
                    print("initiate_distance", self.initiate_distance)
                    # initiate_distance {'target': {path_index1: [{slot1: distance, slot2: distance}], path_index2: [{slot1: distance}]}}
                    # {'251': {0: [{1: 0, 2: 0}], 1: [{1: 0}]}}
                    for _ranges in self.state_distance[target][path_index][slot]:
                        for bound in _ranges:
                            if bound not in (math.inf, -math.inf):
                                self.interesting_value.add(int(bound))


        print("self.interesting_value:", self.interesting_value)
    ...
    def execute(self, population, engine):
        ...
        for i, individual in enumerate(population.individuals):
            ...
            code_distances.append(individual.code_distance)
            code_score = f"seed {i} code {individual.code_distance}"
            print(code_score)
            f.write(code_score)
            f.write('\n')
            state_distances.append(individual.state_distance)
            state_score = f"seed {i} state {individual.state_distance}"
            print(state_score)
            f.write(state_score)
            f.write('\n')
            print("----------------------------------------------")

            # TODO: print seed score

        f.close()

        code_std = np.std(code_distances)
        state_std = np.std(state_distances)

        for index, individual in enumerate(population.individuals):
            if code_std:
                normalized_code_distance = individual.code_distance / code_std
            else:
                normalized_code_distance = 1
            if state_std:
                normalized_state_distance = individual.state_distance / state_std
            else:
                normalized_state_distance = 1
            individual.normalized_score = float(1 / (0.5 * normalized_code_distance + 0.5 * normalized_state_distance
                                                     + 0.1))
            print(f"seed:{index} get score of {fitness_function(individual, self.env)}")
            #print(f"seed:{index} get score of {individual.normalized_score}")
    ...
    def register_step(self, g, population, engine):
        ...
        # print(population.indv_generator.arguments_pool)
        # TODO: print pool

        with open('coverage.txt','a')as f:
            _msg = 'Generation number {} \t Code coverage: {:.2f}% \t Branch coverage: {:.2f}% \n'.format(
            g + 1, code_coverage_percentage, branch_coverage_percentage)
            f.write(_msg)
    ...
    def execution_function(self, indv, env: FuzzingEnvironment):
        ...
        mutated_arguments = set()

        current_distance = copy.deepcopy(self.initiate_distance)
        # print("indv.solution\n")
        # print(indv.solution)

        for transaction_index, test in enumerate(indv.solution):

            # print((transaction_index, test))
            ...
            basic_block_distances = []

            for i, instruction in enumerate(result.trace):

                # print((transaction_index, i, instruction))
                if str(instruction["pc"]) in self.code_distance:
                    if self.code_distance[str(instruction["pc"])]["distance"] not in (math.inf, 0.001):
                        basic_block_distances.append(self.code_distance[str(instruction["pc"])]["distance"])

                '''for Distance in self.code_distance:
                    if instruction["pc"] == Distance["node"]:
                        # do not take these into account:
                        # 1. basic blocks that cannot reach any of the target blocks
                        # 2. basic blocks that follows target blocks
                        if Distance["distance"] != math.inf and Distance["distance"] != 0.001:
                            basic_block_distances += Distance["distance"]'''
                ...
                # Extract data dependencies (read-after-write)
                    ...
                    if storage_slot in self.interesting_slot:
                        for target in self.state_distance:
                            for path_index, path in enumerate(self.state_distance[target]):
                                if str(storage_slot) in path:
                                    last_trace = current_distance[target][path_index][-1].copy()
                                    if convert_stack_value_to_int(instruction["stack"][-2]) != last_trace[storage_slot]:
                                        current_distance[target][path_index].append(last_trace)
                                        current_distance[target][path_index][-1][storage_slot] = \
                                            convert_stack_value_to_int(instruction["stack"][-2])
                                    #print(current_distance[target][path_index])
                        print("storage",storage_slot, "value",convert_stack_value_to_int(instruction["stack"][-2]))
                        # TODO: print storage

                        if not indv.generator.arguments_pool.get(_function_hash):
                            for argument_index in range(100):
                                try:
                                    if "int" in indv.generator.interface[_function_hash][argument_index]:
                                        for argument in self.interesting_value:
                                            indv.generator.add_argument_to_pool(_function_hash, argument_index, argument)
                                            mutated_arguments.add((_function_hash, argument_index, argument))
                                except IndexError:
                                    break
                                try:
                                    if "uint" in indv.generator.interface[_function_hash][argument_index]:
                                        for argument in self.interesting_value:
                                            indv.generator.add_argument_to_pool(_function_hash, argument_index, argument)
                                            mutated_arguments.add((_function_hash, argument_index, argument))
                                except IndexError:
            ...
            if not basic_block_distances:
                transaction_distance.append(100)
            else:
                set_min = 5

                if len(basic_block_distances) > set_min:
                    transaction_distance.append(np.mean(sorted(basic_block_distances)[:set_min]))
                else:
                    transaction_distance.append(np.mean(basic_block_distances))

        state_distance = 0
        find_zero = False
        for target in self.state_distance:
            # minimum of all the path toward this target
            min_state = math.inf
            for path_index, path in enumerate(self.state_distance[target]):
                for results in current_distance[target][path_index]:
                    current_state = 0
                    for slot in results:
                        current_state += var_distance(results[slot], self.state_distance[target][path_index][str(slot)])
                    if current_state == 0:
                        find_zero = True
                        break
                    elif current_state < min_state:
                        min_state = current_state
                    # print(current_state, results[slot], results)
                if find_zero:
                    break
            if find_zero:
                break
            else:
                state_distance += 1 / min_state

        if find_zero or not self.state_distance or not state_distance:
            indv.state_distance = 1
        else:
            indv.state_distance = len(self.state_distance) / state_distance

            for mutate_argument in mutated_arguments:
                if random.random() <= 0.1:
                    indv.generator.add_argument_to_pool(mutate_argument[0], mutate_argument[1],
                                                        int(mutate_argument[2] + indv.state_distance))
                    if indv.generator.interface[mutate_argument[0]][mutate_argument[1]].startswith("int") \
                            or int(mutate_argument[2] - indv.state_distance) >= 0:
                        indv.generator.add_argument_to_pool(mutate_argument[0], mutate_argument[1],
                                                            int(mutate_argument[2] - indv.state_distance))
                    else:
                        indv.generator.add_argument_to_pool(mutate_argument[0], mutate_argument[1], 0)

        transaction_distance.sort()

        try:
            indv.code_distance = transaction_distance[0]
        except:
            indv.code_distance = 100
            ...
            if check == sat:
                ...
                for variable in model:
                    ...
                    if str(variable).startswith("balance"):
                        ...
                        indv_generator.add_returndatasize_to_pool(_function_hash, _address,
                                                                  int(model[variable].as_long()))
