...
class EvolutionaryFuzzingEngine(object):
    ...
    def __init__(self, population, selection, crossover, mutation, fitness=None, analysis=None, mapping=None, srcmap = None):
        # Set logger.
    ...
    def run(self, ng):
        ...
        try:
            ...
            with open('res.txt', 'a')as f:
                ...
                f.write(f'      start fuzzing contract:{str(self.srcmap.source.filename)}\n')
            ...
            while g < ng or settings.GLOBAL_TIMEOUT:
                ...
                print('prvious generation:')
                _indvs = self.population.individuals
                for index, indv in enumerate(_indvs):
                    print('========================')
                    print(f'seed {index}:')
                    _chromosome = indv.chromosome
                    for tx, item in enumerate(_chromosome):
                        #print(f'tx{tx}:')
                ...
                for _ in range(size):
                    ...
                    for index, indv in enumerate(_indvs):
                        if parents[0].chromosome == indv.chromosome:
                            print(f'choose seed:{index} as father')
                            break
                    
                    # for index, item in enumerate(parents[0].chromosome):
                    #     print(f'tx{index}')
                    #     print(item['arguments'])


                    # print('choose mather:')
                    # for index, item in enumerate(parents[1].chromosome):
                    #     print(f'tx{index}')
                    #     print(item['arguments'])


                    for index, indv in enumerate(_indvs):
                        if parents[1].chromosome == indv.chromosome:
                            print(f'choose seed:{index} as mather')
                            print(f'-------------')
                            break
                ...
                for a in self.analysis:
                    ...
                    # print("\n\n\n\n")
                    # print(a.env.detector_executor.locking_ether_detector.severity)
                    # print("\n\n\n\n")
            ...
            #TODO
            # print("can_recv_ether：",gl.get_can_recv_ether())
            # print("\n\n\n\n")
            # print("can_send_ether：",gl.get_can_send_ether())
            # print("\n\n\n\n")

            # print(gl.get_can_recv_ether())
            # print(gl.get_can_send_ether())
            # print("\n\n\n\n")

            # if gl.get_can_recv_ether() == True and gl.get_can_send_ether() == False:
            #     lock_ether()
            #TODO
            for a in self.analysis:
                # print(a.env.detector_executor.locking_ether_detector.can_recv)
                # print(a.env.detector_executor.locking_ether_detector.can_send)
                # print("\n\n\n\n")
                if a.env.detector_executor.locking_ether_detector.can_recv == True and a.env.detector_executor.locking_ether_detector.can_send == False:
                    lock_ether(self.srcmap, a.env)
...
#TODO
def lock_ether(srcmap, env):
    detector = detectors.DetectorExecutor()
    detector.initialize_detectors()
    color = "\u001b[33m"
    detector.logger.title(color+"-----------------------------------------------------")
    detector.logger.title(color+"           !!!! Locking ether detected !!!            ")
    detector.logger.title(color+"-----------------------------------------------------")
    detector.logger.title(color+"SWC-ID:   "+str(detector.locking_ether_detector.swc_id))
    detector.logger.title(color+"Severity: "+detector.locking_ether_detector.severity)
    detector.logger.title(color+"-----------------------------------------------------")
    detectors.print_res(str(srcmap.source.filename), 'None', 'Lock Ether', env)
